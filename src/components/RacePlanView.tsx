      ...
      <div className={isFirstBuild ? "w-full rounded-2xl border border-border bg-surface p-6" : "w-full max-w-lg rounded-2xl border border-border bg-background p-6 shadow-xl"}>
        ...
        {step===1 && (
          <div className="space-y-4">
            ...
            <div><label className="text-xs text-foreground-dim uppercase tracking-wide mb-1 block">Plan start date</label><input type="date" min={todayStr} value={form.startDate} onChange={e=>setForm({...form,startDate:e.target.value})} className="w-full px-3 py-2 rounded-xl bg-surface border border-border text-sm outline-none focus:border-signal"/><p className="text-xs text-foreground-dim mt-1">Your plan will start the Monday of that week.</p></div>
