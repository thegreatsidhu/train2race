...
function WorkoutModal({ workout, onClose, onLogged }: { workout: any; onClose: () => void; onLogged: () => void }) {
  ...
  async function handleMove() {
    ...
    await fetch("/api/races/workouts/" + wid + "/move", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ newDate: moveDate }) });
    setMoving(false);
    setShowMove(false);
    onMoved?.();
    onClose();
      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-background p-6 shadow-xl">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-foreground-dim mt-0.5">{workout.day} ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· Week {workout.week}{workout.distanceKm ? ` ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· ${(workout.distanceKm/1.60934).toFixed(1)} mi planned` : ""}</p>
          </div>
        ...
        {!workout.completed && (
          <div className="mb-4 border-t border-border pt-4">
            {!showMove ? (
              <button onClick={() => setShowMove(true)} className="text-xs text-foreground-dim hover:text-foreground transition-colors">Move to a different day</button>
            ) : (
              <div className="rounded-xl border border-border bg-surface p-3">
                <p className="text-xs text-foreground-dim mb-2">Pick a new date for this workout:</p>
        ...
        {workout.completed ? <p className="text-sm text-signal text-center font-medium py-2">Logged to your activity feed</p> : (
          <>
            <p className="text-xs text-foreground-dim uppercase tracking-wide mb-3">Log this workout</p>
            <div className="mb-4"><label className="text-xs text-foreground-dim mb-2 block">How did it feel?</label><div className="flex flex-wrap gap-2">{FEEL_OPTIONS.map(opt => <button key={opt.value} onClick={() => setForm({...form,feel:opt.value})} className={"text-xs px-3 py-1.5 rounded-full border transition-colors " + (form.feel===opt.value ? "bg-signal text-background border-signal" : "border-border hover:bg-surface")}>{opt.label}</button>)}</div></div>
            <div className="mb-4"><label className="text-xs text-foreground-dim mb-1 block">Effort (RPE): {form.effort}</label><input type="range" min={1} max={7} value={form.effort} onChange={e => setForm({...form,effort:Number(e.target.value)})} className="w-full accent-signal" /><div className="flex justify-between text-xs text-foreground-dim"><span>Easy</span><span>Max</span></div></div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div><label className="text-xs text-foreground-dim mb-1 block">Actual distance (mi)</label><input type="number" step="0.1" value={form.actualDistanceMi} onChange={e => setForm({...form,actualDistanceMi:e.target.value})} className="w-full px-3 py-2 rounded-xl bg-surface border border-border text-sm outline-none focus:border-signal" /></div>
              <div><label className="text-xs text-foreground-dim mb-1 block">Actual duration (min)</label><input type="number" value={form.actualDurationMin} onChange={e => setForm({...form,actualDurationMin:e.target.value})} className="w-full px-3 py-2 rounded-xl bg-surface border border-border text-sm outline-none focus:border-signal" /></div>
            </div>
            <div className="mb-5"><label className="text-xs text-foreground-dim mb-1 block">Notes (optional)</label><textarea rows={2} value={form.notes} onChange={e => setForm({...form,notes:e.target.value})} className="w-full px-3 py-2 rounded-xl bg-surface border border-border text-sm outline-none focus:border-signal resize-none" /></div>
            <button onClick={handleLog} disabled={logging||submitted} className="w-full py-3 rounded-full bg-signal text-background font-medium text-sm disabled:opacity-60">{submitted ? "Logged!" : logging ? "Saving..." : "Log workout & mark done"}</button>
          </>
        )}
      </div>
    </div>
...
function BuildPlanForm({ race, onBuilt }: { race: any; onBuilt: () => void }) {
  ...
  async function handleBuild() {
    ...
    try {
      const res = await fetch(`/api/races/${race.id}/generate-plan`, { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ weeklyMileageKm: form.currentWeeklyMileage ? Number(form.currentWeeklyMileage)*1.60934 : race.weeklyMileageKm, recentRaceTime: form.recentRaceTime, trainingDaysPerWeek: Number(form.trainingDaysPerWeek), raceType: race.raceType, isTriathlon: race.isTriathlon, hardDays: form.hardDays, longRunDay: form.longRunDay, injuryConcerns: form.injuryConcerns, fitnessNotes: form.fitnessNotes, prioritize: form.prioritize }) });
      if (!res.ok) { const d = await res.json(); setError(d.error||"Failed"); } else { onBuilt(); }
    } catch(e: any) { setError(e.message); }
    setGenerating(false);
  }
  if (generating) return <div className="rounded-2xl border border-border bg-surface p-8 text-center"><div className="w-8 h-8 border-2 border-signal border-t-transparent rounded-full animate-spin mx-auto mb-4" /><p className="text-sm font-medium">Building your plan...</p><p className="text-xs text-foreground-dim mt-1">This takes about 15-30 seconds</p></div>;
  return (
    <div className="rounded-2xl border border-border bg-surface p-6">
      <div className="flex items-center gap-2 mb-5">{[1,2,3].map(s => <div key={s} className={"h-1.5 rounded-full transition-all " + (s===step?"w-6 bg-signal":s<step?"w-3 bg-signal opacity-50":"w-3 bg-border")} />)}<span className="text-xs text-foreground-dim ml-1">Step {step} of 3</span></div>
      {error && <p className="text-red-400 text-sm mb-4">{error}</p>}
      {step===1 && <div className="space-y-4">
        <h2 className="text-sm font-medium">Your current fitness</h2>
        <div><label className="text-xs text-foreground-dim mb-1 block">Current weekly mileage (miles)</label><input type="number" placeholder="e.g. 20" value={form.currentWeeklyMileage} onChange={e=>setForm({...form,currentWeeklyMileage:e.target.value})} className="w-full px-3 py-2 rounded-xl bg-background border border-border text-sm outline-none focus:border-signal" /></div>
        ...
        <button onClick={()=>setStep(2)} className="w-full py-2.5 rounded-full bg-signal text-background text-sm font-medium">Next ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢</button>
      </div>}
      {step===2 && <div className="space-y-4">
        ...
        <div className="flex gap-3"><button onClick={()=>setStep(1)} className="flex-1 py-2.5 rounded-full border border-border text-sm hover:bg-surface">ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Back</button><button onClick={()=>setStep(3)} className="flex-1 py-2.5 rounded-full bg-signal text-background text-sm font-medium">Next ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢</button></div>
      </div>}
      {step===3 && <div className="space-y-4">
        ...
        <div className="rounded-xl bg-surface-raised border border-border p-3"><p className="text-xs text-foreground-dim uppercase tracking-wide mb-1">Plan summary</p><p className="text-sm">{form.currentWeeklyMileage||"?"} mi/week ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· {form.trainingDaysPerWeek} days ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· {form.prioritize}{form.longRunDay?` ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· Long run ${form.longRunDay}`:""}{form.hardDays.length>0?` ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· Hard: ${form.hardDays.map(d=>d.slice(0,3)).join(", ")}`:""}</p></div>
        <div className="flex gap-3"><button onClick={()=>setStep(2)} className="flex-1 py-2.5 rounded-full border border-border text-sm hover:bg-surface">ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Back</button><button onClick={handleBuild} className="flex-1 py-2.5 rounded-full bg-signal text-background text-sm font-medium">Build my plan ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢</button></div>
      </div>}
    </div>
  );
}
    ...
    <div>
      ...
      <div className="space-y-6">
        {Object.entries(weeks).map(([weekNum,workouts]) => {
          const weekDone = (workouts as any[]).filter(w=>w.completed).length;
          const isCurrentWeek = (workouts as any[]).some(w => { const d=new Date(w.date); d.setHours(0,0,0,0); const diff=(d.getTime()-today.getTime())/(1000*60*60*24); return diff>=0&&diff<7; });
          return (
        ...
        {workout.completed ? <div className="mt-1 w-full text-xs py-1 rounded-lg text-center bg-signal text-background">Done</div>
                       : isPast ? <button onClick={()=>setSelectedWorkout(workout)} className="mt-1 w-full text-xs py-1 rounded-lg border border-border hover:bg-surface-raised transition-colors">Log it</button>
                       : null}
                    </div>
                  );
                })}
