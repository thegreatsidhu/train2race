...
export default async function TodayPage() {
  ...
  const loggedItems = recentActivities.map((a: any) => ({
    ...
    durationSec: a.durationSec, distanceM: a.distanceM, source: a.source, raw: a.raw, photos: a.photos ?? [],
  }));
    ...
    <div className="max-w-2xl px-4 md:px-8 py-6 md:py-10">
      ...
      {teamsWithActivity.length > 0 ? (
        <section className="mb-6">
          ...
          {teamsWithActivity.length > 5 && (
            <Link href="/dashboard/teams" className="inline-block mt-2 text-xs text-foreground-dim hover:text-signal transition-colors">
              See all {teamsWithActivity.length} teams →
            </Link>
          )}
        </section>
      ) : !isNewUser && (
