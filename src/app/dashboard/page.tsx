...
export default async function TodayPage() {
  ...
  const [history, hasConnection, recentActivities, activeRace, weeklyActivities] = await Promise.all([
    ...
    prisma.deviceConnection.findFirst({ where: { userId }, select: { id: true, source: true } }),
  ...
  const weeklyMiles = weeklyActivities.reduce((sum, a) => sum+(a.distanceM||0)/1609.34, 0);
  const weeklyTime = weeklyActivities.reduce((sum, a) => sum+(a.durationSec||0), 0);
    ...
    <div className="max-w-4xl px-4 md:px-8 py-6 md:py-10">
        ...
        <section className="mb-6">
          <div className="rounded-2xl border border-border bg-surface overflow-hidden">
            <div className="px-5 py-4 border-b border-border flex items-center justify-between">
              <div><p className="text-xs text-foreground-dim uppercase tracking-wide mb-0.5">Training for</p><h2 className="font-semibold">{activeRace.raceName}</h2><p className="text-xs text-foreground-dim mt-0.5">{daysToRace} day{daysToRace===1?"":"s"} away · {(activeRace.distanceM/1609.34).toFixed(1)} mi</p></div>
                ...
                <div>
                  ...
                  <div className={`rounded-xl border p-3 ${TYPE_COLORS[todaysWorkout.type]||"bg-surface border-border"} ${todaysWorkout.completed?"opacity-50":""}`}>
                    <div className="flex items-center justify-between">
                      ...
                      {todaysWorkout.completed?<span className="text-xs bg-signal/20 text-signal px-2 py-1 rounded-full">Done</span>:<Link href={`/dashboard/races/${activeRace.id}`} className="text-xs bg-background/30 hover:bg-background/50 px-3 py-1.5 rounded-full transition-colors">Log it</Link>}
                    </div>
                  </div>
                </div>
      ...
      {!hasConnection && (
        <div className="rounded-2xl border border-border bg-surface p-6 mb-6">
          ...
          <p className="text-sm text-foreground-dim mb-3">Connect your wearable to see your recovery metrics and training load.</p>
      ...
      {hasData && (
        <>
          ...
          <section className="mb-6">
            <div className="rounded-2xl border border-border bg-surface p-4 md:p-6">
              <div className="flex items-center justify-between mb-3">
                <div><span className="font-data text-xs text-foreground-dim uppercase tracking-wide">Resting heart rate</span>{rhrComparison&&<span className={`ml-2 text-xs ${rhrComparison.direction==="down"?"text-signal":rhrComparison.direction==="up"?"text-alert":"text-foreground-dim"}`}>{rhrComparison.direction==="down"?"↓":rhrComparison.direction==="up"?"↑":"→"} {rhrComparison.deltaPct?Math.abs(rhrComparison.deltaPct)+"% vs 30d":""}</span>}</div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3"><h2 className="text-sm font-medium text-foreground-dim">Recent activity</h2><Link href="/dashboard/log-workout" className="px-4 py-2 rounded-full bg-signal text-background text-sm font-medium hover:bg-signal-dim transition-colors">+ Log workout</Link></div>
