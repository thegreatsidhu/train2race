            ...
            {challenges.map(c=>{
              ...
              return(
                <div key={c.id} className={"rounded-2xl border bg-surface p-5 "+(isPending?"border-yellow-700/40 opacity-80":isRejected?"border-red-700/30 opacity-60":"border-border")}>
                  ...
                  {pct!==null&&!isPending&&!isRejected&&<div className="mb-3">{pct>=100?<div className="flex items-center gap-2 mb-1"><span className="text-xs px-2 py-0.5 rounded-full bg-yellow-900/30 text-yellow-300 border border-yellow-700/40 font-medium">🏆 Challenge completed!</span></div>:<div className="flex justify-between text-xs text-foreground-dim mb-1"><span>My progress</span><span>{myTotal} / {c.goal} {c.unit} ({pct}%)</span></div>}<div className="w-full h-2 bg-border rounded-full"><div className={"h-2 rounded-full transition-all "+(pct>=100?"bg-yellow-400":"bg-signal")} style={{width:`${Math.min(pct,100)}%`}}/></div></div>}
