...
export default function TeamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function leaveChallenge(cId:string,title:string){if(!confirm(`Clear your entries for "${title}"? This removes your logged progress.`))return;setLeavingChallenge(cId);const res=await fetch(`/api/teams/${id}/challenges/${cId}/entries`,{method:"DELETE"});if(res.ok){setChallenges(prev=>prev.map(c=>c.id===cId?{...c,entries:c.entries.filter((e:any)=>e.userId!==myUserId)}:c));}setLeavingChallenge(null);}
                ...
                <div key={c.id} className={"rounded-2xl border bg-surface p-5 "+(isPending?"border-yellow-700/40 opacity-80":isRejected?"border-red-700/30 opacity-60":"border-border")}>
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-3 shrink-0 ml-3">
                      ...
                      {!isPending&&!isRejected&&<button onClick={()=>leaveChallenge(c.id,c.title)} disabled={leavingChallenge===c.id} className="text-xs text-foreground-dim hover:text-red-400 hover:underline disabled:opacity-40">{leavingChallenge===c.id?"...":"Leave"}</button>}
