...
export default function TeamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function onMetricChange(metric:string){const firstUnit=metric==="count"?countUnitsFor(challengeForm.type)[0]:METRIC_UNITS[metric]?.[0]??"mi";setChallengeForm(f=>({...f,metric,unit:firstUnit,goalPerDay:metric!=="count"?false:f.goalPerDay}));}
          ...
          {showNewChallenge?(
            <div className="rounded-2xl border border-border bg-surface p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="text-xs text-foreground-dim uppercase tracking-wide mb-1 block">Goal</label><input type="number" className="w-full bg-background border border-border rounded-xl px-3 py-2 text-sm" placeholder="Required" value={challengeForm.goal} onChange={e=>setChallengeForm(f=>({...f,goal:e.target.value}))}/>{challengeForm.metric==="count"&&<label className="flex items-center gap-1.5 mt-1.5 cursor-pointer text-xs text-foreground-dim"><input type="checkbox" checked={challengeForm.goalPerDay} onChange={e=>setChallengeForm(f=>({...f,goalPerDay:e.target.checked}))} className="rounded"/>{challengeForm.goalPerDay?"Per day goal":"Make it a per day goal"}</label>}</div>
                ...
                <div key={c.id} className={"rounded-2xl border bg-surface p-5 "+(isPending?"border-yellow-700/40 opacity-80":isRejected?"border-red-700/30 opacity-60":"border-border")}>
                  ...
                  {editingChallengeId===c.id&&<div className="mb-3 p-3 rounded-xl bg-background border border-border space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <select className="bg-surface border border-border rounded-lg px-3 py-1.5 text-sm" value={editChallengeForm.type} onChange={e=>{const t=e.target.value;if(t==="walk"){const u=countUnitsFor("walk")[0];setEditChallengeForm(f=>({...f,type:t,metric:"count",unit:u}));}else setEditChallengeForm(f=>({...f,type:t,unit:f.metric==="count"?"sessions":f.unit,goalPerDay:false}));}}><option value="run">Run</option><option value="walk">Walk</option><option value="swim">Swim</option><option value="bike">Bike</option><option value="custom">Custom</option></select>
                      <select className="bg-surface border border-border rounded-lg px-3 py-1.5 text-sm" value={editChallengeForm.metric} onChange={e=>{const m=e.target.value;const u=m==="count"?countUnitsFor(editChallengeForm.type)[0]:METRIC_UNITS[m]?.[0]??"mi";setEditChallengeForm(f=>({...f,metric:m,unit:u,goalPerDay:m!=="count"?false:f.goalPerDay}));}}><option value="distance">Distance</option><option value="duration">Duration</option><option value="count">Count</option></select>
                      ...
                      <div><input type="number" className="w-full bg-surface border border-border rounded-lg px-3 py-1.5 text-sm" placeholder="Goal" value={editChallengeForm.goal} onChange={e=>setEditChallengeForm(f=>({...f,goal:e.target.value}))}/>{editChallengeForm.metric==="count"&&<label className="flex items-center gap-1.5 mt-1 cursor-pointer text-xs text-foreground-dim"><input type="checkbox" checked={editChallengeForm.goalPerDay} onChange={e=>setEditChallengeForm(f=>({...f,goalPerDay:e.target.checked}))} className="rounded"/>Per day goal</label>}</div>
