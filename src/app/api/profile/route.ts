...
export async function GET() {
  ...
  const user = await (prisma as any).user.findUnique({ where: { id: userId }, select: { name: true, email: true, weightKg: true, heightCm: true, dateOfBirth: true, sex: true, city: true, isPrivate: true, timezone: true, bio: true, emailOptOut: true, passwordHash: true } });
...
export async function POST(req: Request) {
  ...
  const { name, weightKg, heightCm, dateOfBirth, sex, city, isPrivate, timezone, bio, emailOptOut } = body;
  const data: any = {};
  ...
  if ("emailOptOut" in body) data.emailOptOut = !!emailOptOut;
