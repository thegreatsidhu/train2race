...
export async function GET(req: NextRequest) {
  ...
  const [deletedMetrics, deletedTeamMsgs, deletedRejected] = await Promise.all([
  ...
  return NextResponse.json({ ok: true, cleaned: { metrics: deletedMetrics.count, teamMessages: deletedTeamMsgs.count, rejectedChallenges: deletedRejected.count }, weeklyEmails, digestEmails, editNotifs, inviteNotifs, awardsGenerated, announcementsGenerated, ranAt: new Date().toISOString() });
}
