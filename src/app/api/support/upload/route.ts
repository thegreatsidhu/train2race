...
export async function POST(req: NextRequest) {
  ...
  if (!ALLOWED.has(file.type)) return NextResponse.json({ error: "File must be JPG, PNG, or GIF" }, { status: 400 });
  if (file.size > MAX_BYTES) return NextResponse.json({ error: "File must be under 5 MB" }, { status: 400 });

  const key = `tickets/${userId}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext(file.type)}`;
