...
// Access to this component is gated server-side in page.tsx (session role check, 404 for
// everyone else) — there is no password screen here anymore. `password` is kept as an
// always-empty string purely so the many existing fetch calls below that still include it
// in their request bodies/query strings don't need to change; the server ignores it.
export function AdminPanel() {
  ...
  function loadAdminData() {
    ...
    fetch("/api/admin", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "getData" }) })
      ...
      .catch(() => { setLoadError("Failed to load admin data."); setLoading(false); });
  }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="w-full max-w-sm text-center">
          <p className="text-red-400 text-sm mb-3">{loadError || "Failed to load admin data."}</p>
          <button onClick={loadAdminData} className="px-5 py-2.5 rounded-full bg-signal text-background text-sm font-medium">Retry</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => signOut({ callbackUrl: "/" })} className="px-4 py-2 rounded-full border border-border text-sm">Sign out</button>
        </div>
